...
def gerar_enxame_aleatorio():
    enxame = []
    permutacoes_adicionadas = []
    while len(enxame) < num_particulas:
        candidato = random.sample(range(num_jobs), num_jobs)
        if all(distancia_permutacao(candidato, outro) >= dist_minima for outro in permutacoes_adicionadas):
            particula = Particula(candidato, len(enxame))
            enxame.append(particula)
            permutacoes_adicionadas.append(candidato)
    return enxame
...
def executar_pso_discreto(enxame, tempo_processamento):
    ...
    return melhor_global_posicao, melhor_global_valor, [p.melhor_posicao for p in enxame]
...
def main():
    ...
    enxame_inicial = gerar_enxame_aleatorio()
    solucao_pso, makespan_pso, melhores_particulas = executar_pso_discreto(enxame_inicial, tempo_processamento)

    # Pega as melhores partículas para iniciar o genético
    populacao_inicial = sorted(melhores_particulas, key=lambda p: calcular_makespan(tempo_processamento, p))[:tamanho_populacao]
    solucao_final, makespan_final = algoritmo_genetico(populacao_inicial)
    ...
    print("\n--- Genético ---")
    print("Melhor solução final:", solucao_final)
